...
exports.handleExpiredSlots = () => {
    const query = `
        SELECT id, username, slot, search_term, display_keyword, created_at, note 
        FROM registrations
        WHERE slot > (SELECT remainingSlots FROM users WHERE username = registrations.username)
        ORDER BY created_at ASC
    `;

    connection.query(query, (err, results) => {
        if (err) {
            console.error('Error fetching expired slots:', err);
            return;
        }
        ...
        results.forEach((keyword) => {
            ...
            connection.query(insertDeletedQuery, [
                ...
                // 기존 registrations 테이블에서 키워드 삭제
                const deleteQuery = `DELETE FROM registrations WHERE id = ? AND username = ?`;
                connection.query(deleteQuery, [keyword.id, keyword.username], (err) => {
                    if (err) {
                        console.error('Error deleting keyword:', err);
                        return;
                    }

                    console.log(`Keyword ${keyword.search_term} deleted and moved to deleted_keywords table.`);
                });
            });
        });
    });
...
// 3일이 지난 삭제된 키워드 영구 삭제 함수 추가
exports.deleteOldDeletedKeywords = () => {
    const query = `
        DELETE FROM deleted_keywords
        WHERE deleted_at < DATE_SUB(NOW(), INTERVAL 3 DAY)
    `;

    connection.query(query, (err, results) => {
        if (err) {
            console.error('Error deleting old deleted keywords:', err);
            return;
        }

        console.log(`Deleted ${results.affectedRows} old deleted keywords.`);
    });
};
